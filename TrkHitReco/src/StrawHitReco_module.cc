...
StrawHitReco::StrawHitReco(fhicl::ParameterSet const& pset)
    : art::EDProducer{pset},
      ...
      _usecc(pset.get<bool>("UseCalorimeter", false)), _clusterDt(pset.get<float>("clusterDt", 100)),
      ...
      _filter(pset.get<bool>("FilterHits")), _writesh(pset.get<bool>("WriteStrawHitCollection")),
      _flagXT(pset.get<bool>("FlagCrossTalk", false)), _printLevel(pset.get<int>("printLevel", 0)),
      _diagLevel(pset.get<int>("diagLevel", 0)), _end{StrawEnd::cal,
                                                      StrawEnd::hv}, // this should be in a general place, FIXME!
      ...
      _peakfit(pset.get<fhicl::ParameterSet>("PeakFitter", {}))
{
    ...
    if (_printLevel > 0)
        std::cout << "In StrawHitReco constructor " << std::endl;
}
...
void StrawHitReco::beginRun(art::Run& run)
{
    ...
    if (_printLevel > 0)
        std::cout << "In StrawHitReco begin Run " << std::endl;
}
...
{
    if (_printLevel > 0)
        std::cout << "In StrawHitReco produce " << std::endl;
    ...
    for (size_t isd = 0; isd < sdcol.size(); ++isd) {
        ...
        if (_usecc && caloClusters) {
            ...
            for (const auto& cluster : *caloClusters)
                if (std::abs(time - cluster.time()) < _clusterDt) {
                    outsideCaloTime = false;
                    break;
                }
            if (outsideCaloTime) {
                if (_filter)
        ...
        if (_fittype == TrkHitReco::FitType::peakminuspedavg) {
            ...
            if (_printLevel > 1)
                std::cout << "Fit status = " << params._status << " NDF = " << params._ndf << " chisquared "
                          << params._chi2 << " Fit charge = " << params._charge << " Fit time = " << params._time
                          << std::endl;
        }
        // energy selection
        ...
        ch._flag = flag;
        if (td)
            ch._flag.merge(StrawHitFlag::tdiv);
        ch._tend = eend;
        if (!_filter && _flagXT) {
            ...
            if (energy >= _ctE) {
                largeHits.push_back(shCol->size());
                largeHitPanels.push_back(global_panel);
            }
        }
...
float StrawHitReco::peakMinusPedAvg(TrkTypes::ADCWaveform const& adcData) const
{
    ...
    float peak = *maxIter;
    if (_diagLevel > 0)
        _maxiter->Fill(std::distance(wfstart, maxIter));
