...
KalSeedFit::KalSeedFit(fhicl::ParameterSet const &pset)
  ...
  if (_perr.size() != HelixTraj::NHLXPRM)
    throw cet::exception("RECO")
        << "mu2e::KalSeedFit: parameter error vector has wrong size" << endl;
...
void KalSeedFit::produce(art::Event &event) {
  ...
  for (size_t iseed = 0; iseed < _hscol->size(); ++iseed) {
    ...
    if (_helicity != hseed.helix().helicity()) {
      if (_checkhelicity)
        throw cet::exception("RECO")
            << "mu2e::KalSeedFit: helicity doesn't match configuration" << endl;
    ...
    if (hseed.status().hasAllProperties(_seedflag) &&
      ...
      // KalSeed
      int nsh =
          seeddef.strawHitIndices().size(); // tclust._strawHitIdxs.size();
      for (int i = 0; i < nsh; ++i) {
      ...
      if (_result.krep != 0 &&
        ...
        KalSeed kseed(_result.krep->particleType(), _fdir, _result.krep->t0(),
                      _result.krep->flt0(), kf.status());
        ...
        const HelixTraj *htraj = dynamic_cast<const HelixTraj *>(
            _result.krep->localTrajectory(_result.krep->flt0(), locflt));
        // use this to create segment.  This will be the only segment in
        // this track
        if (htraj != 0) {
          KalSegment kseg;
          ...
          TrkUtilities::fillSegment(*htraj, momerr,
                                    locflt - _result.krep->flt0(), kseg);
          // extend the segment
