...
struct StrawHit {
  ...
  public:
    StrawHit() : _strawId(StrawId(-1)), _time{{0.0, 0.0}}, _tot{{0.0, 0.0}}, _energyDep(0.) {}

    // Constructor for a hit that came from an unpacked digi, either
    // from data or from the full MC chain.
    StrawHit(StrawId strawId, TrkTypes::TDCTimes const& time, TrkTypes::TOTTimes const& tot, float energyDep)
        : _strawId(strawId), _time(time), _tot(tot), _energyDep(energyDep)
    {
    }
    ...
    {
        return (_strawId == other._strawId && _time[0] == other._time[0] && _time[1] == other._time[1] &&
                _tot[0] == other._tot[0] && _tot[1] == other._tot[1] && _energyDep == other._energyDep);
    }
    bool operator<(const StrawHit other) const { return (_strawId < other._strawId); }
    bool operator>(const StrawHit other) const { return (_strawId > other._strawId); }
    // Print contents of the object.
    void print(std::ostream& ost = std::cout, bool doEndl = true) const;
};
inline std::ostream& operator<<(std::ostream& ost, StrawHit const& hit)
{
    hit.print(ost, false);
    return ost;
}
