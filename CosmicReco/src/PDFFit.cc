...
FullDriftFit::FullDriftFit(ComboHitCollection _chits, StrawResponse const& _srep, CosmicTrack _track,
                           std::vector<double>& _constraint_means, std::vector<double>& _constraints, double _sigma_t,
                           int _k, const Tracker* _tracker)
...
void FullDriftFit::CalculateFullPDF()
{
    ...
    for (int is = 0; is < N_sbins; is++) {
        ...
        for (int it0 = 0; it0 < N_tbins; it0++) {
            ...
            double time_gaussian =
                1.0 / sqrt(2 * TMath::Pi() * sigma * sigma) * exp(-(time_gaus * time_gaus) / (2 * sigma * sigma));
            ...
            for (int itau = 0; itau < N_taubins; itau++) {
                ...
                for (int it1 = 0; it1 < N_tbins - it0; it1++) {
                    ...
                    double val_tau =
                        pow(1 / tau, k) * pow(time_tau, k - 1) * exp(-time_tau / tau) / (double)Factorial(k - 1);
...
double GaussianPDFFit::calculate_DOCA(ComboHit const& chit, double a0, double a1, double b0, double b1,
...
double GaussianPDFFit::calculate_ambig(ComboHit const& chit, double a0, double a1, double b0, double b1,
                                       const Tracker* tracker) const
{
...
double GaussianPDFFit::TimeResidual(double doca, StrawResponse const& srep, double t0, ComboHit const& hit,
                                    const Tracker* tracker) const
{
