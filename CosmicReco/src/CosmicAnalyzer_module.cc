...
class CosmicAnalyzer : public art::EDAnalyzer {
  public:
    struct Config {
        ...
        fhicl::Atom<int> diag{Name("diagLevel"), Comment("set to 1 for info"),
                              1};
        fhicl::Atom<bool> mcdiag{Name("mcdiag"), Comment("set on for MC info"),
                                 true};
        fhicl::Atom<art::InputTag> chtag{
            Name("ComboHitCollection"),
            Comment("tag for combo hit collection")};
        fhicl::Atom<art::InputTag> tctag{
            Name("TimeClusterCollection"),
            Comment("tag for time cluster collection")};
        fhicl::Atom<art::InputTag> costag{
            Name("CosmicTrackSeedCollection"),
            Comment("tag for cosmci track seed collection")};
        fhicl::Atom<art::InputTag> mcdigistag{
            Name("StrawDigiMCCollection"), Comment("StrawDigi collection tag"),
            "makeSD"};
        fhicl::Table<SimParticleTimeOffset::Config> toff{
            Name("TimeOffsets"), Comment("Sim particle time offset ")};
    };
  ...
  private:
    ...
    // Flags:
    Bool_t _StraightTrackInit, _StraightTrackConverged, _StraightTrackOK,
        _hitsOK;
    Int_t _strawid;
    vector<ComboHitInfoMC> _chinfomc;
    CosmicTrackMCInfo FitMC(const StrawDigiMCCollection *&_mcdigis);
    CosmicTrackMCInfo FillDriftMC(ComboHit const &chit, double reco_ambig,
                                  CosmicTrackMCInfo info,
                                  const Tracker *tracker);
    bool findData(const art::Event &evt);
};

CosmicAnalyzer::CosmicAnalyzer(const Parameters &conf)
    : art::EDAnalyzer(conf), _diag(conf().diag()), _mcdiag(conf().mcdiag()),
      _chtag(conf().chtag()), _tctag(conf().tctag()), _costag(conf().costag()),
      _mcdigistag(conf().mcdigistag()), _toff(conf().toff()) {
...
void CosmicAnalyzer::beginJob() {
    ...
    if (_diag > 0) {
        ...
        _cosmic_analysis = tfs->make<TTree>(
            "cosmic_analysis", " Diagnostics for Cosmic Track Fitting");

        // Create branches:
        _cosmic_analysis->Branch("evt", &_evt, "evt/I"); // add event id
        _cosmic_analysis->Branch("nhits", &_nhits, "nhits/I");
        _cosmic_analysis->Branch("StrawHitsInEvent", &_nsh,
                                 "StrawHitsInEvent/I");
        _cosmic_analysis->Branch("ComboHitsInEvent", &_nch,
                                 "ComboHitsInEvent/I");
        _cosmic_analysis->Branch("PanelsCrossedInEvent", &_n_panels,
                                 "PanelsCrossedInEvent/I");
        _cosmic_analysis->Branch("PlanesCrossedInEvent", &_n_planes,
                                 "PlanesCrossedInEvent/I");
        _cosmic_analysis->Branch("StatonsCrossedInEvent", &_n_stations,
                                 "StationsCrossedInEvent/I");
        _cosmic_analysis->Branch("TimeClustersInEvent", &_ntc,
                                 "TimeClusterInEvent/I");
        _cosmic_analysis->Branch("hit_time", &_hit_time, "hit_time/F");
        _cosmic_analysis->Branch("hit_drit_time", &_hit_drift_time,
                                 "hit_drift_time/F");
        _cosmic_analysis->Branch("cluster_time", &_cluster_time,
                                 "cluster_time/F");
        _cosmic_analysis->Branch("dt", &_dt, "dt/F");
        _cosmic_analysis->Branch("hitsOK", &_hitsOK, "hitsOK/B");
        _cosmic_analysis->Branch("StraightTrackInit", &_StraightTrackInit,
                                 "StraightTrackInit/B");
        _cosmic_analysis->Branch("StraightTrackOK", &_StraightTrackOK,
                                 "StraightTrackOK/B");
        _cosmic_analysis->Branch("StraightTrackConverged",
                                 &_StraightTrackConverged,
                                 "StraightTrackConverged/B");

        //--------------------------------Truth----------------------------------------//
        if (_mcdiag) {

            _truea0XYZ = tfs->make<TH1F>("True Track Parameter A0 XYZ",
                                         "True Track Parameter A0 XYZ ", 50,
                                         -10000, 10000);
            _truea0XYZ->GetXaxis()->SetTitle("Track Parameter A0 XYZ");
            _truea0XYZ->SetStats();

            _trueb0XYZ = tfs->make<TH1F>("True Track Parameter B0 XYZ",
                                         "True Track Parameter B0 XYZ ", 50,
                                         -5000, 5000);
            _trueb0XYZ->GetXaxis()->SetTitle("Track Parameter B0 XYZ");
            _trueb0XYZ->SetStats();

            _truea1XYZ =
                tfs->make<TH1F>("True Track Parameter A1 XYZ ",
                                "True Track Parameter A1 XYZ", 50, -5, 5);
            _truea1XYZ->GetXaxis()->SetTitle("Track Parameter A1 XYZ");
            _truea1XYZ->SetStats();

            _trueb1XYZ =
                tfs->make<TH1F>("True Track Parameter B1 XYZ ",
                                "True Track Parameter B1 XYZ ", 50, -5, 5);
            _trueb1XYZ->GetXaxis()->SetTitle("Track Parameter B1");
            _trueb1XYZ->SetStats();

            _mc_phi_angle =
                tfs->make<TH1F>("#phi_{true, fit}", "#phi_{true, fit}", 100,
                                -3.141529, 3.141529);
            _mc_phi_angle->GetXaxis()->SetTitle("#phi_{true, fit} [rad]");

            _mc_theta_angle = tfs->make<TH1F>(
                "#theta_{true, fit}", "#theta_{true, fit}", 20, 0, 3.141529);
            _mc_theta_angle->GetXaxis()->SetTitle("#theta_{true, fit} [rad]");

            _AMBIG = tfs->make<TH2F>("True v Reco", "True v Reco", 2, -2, 2, 2,
                                     -2, 2);
            ...
            _TrueTimeResiduals = tfs->make<TH1F>("True Time Res ",
                                                 "True Time Resid", 50, 0, 500);
            _TrueTimeResiduals->GetXaxis()->SetTitle("True Time Res [ns]");
            _TrueTimeResiduals->SetStats();

            _A0pull_v_theta_true = tfs->make<TProfile>(
                "Seed Parameter Pull A_{0} v #theta_{true,fit}  ",
                "Seed Parameter Pull A_{0} v #theta_{true,fit} ", 100, 0,
                3.141529, -200, 200);
            _A0pull_v_theta_true->GetXaxis()->SetTitle("#theta_{true}");
            _A0pull_v_theta_true->GetYaxis()->SetTitle("Parameter Pull A_{0}");

            _B0pull_v_theta_true = tfs->make<TProfile>(
                "Seed Parameter Pull B_{0} v #theta_{true,fit}  ",
                "Seed Parameter Pull B_{0} v #theta_{true,fit} ", 100, 0,
                3.141529, -200, 200);
            _B0pull_v_theta_true->GetXaxis()->SetTitle("#theta_{true}");
            _B0pull_v_theta_true->GetYaxis()->SetTitle("Parameter Pull B_{0}");

            _A1pull_v_theta_true = tfs->make<TProfile>(
                "Seed Parameter Pull A_{1} v #theta_{true,fit}  ",
                "Seed Parameter Pull A_{1} v #theta_{true,fit} ", 100, 0,
                3.141529, -10, 10);
            _A1pull_v_theta_true->GetXaxis()->SetTitle("#theta_{true}");
            _A1pull_v_theta_true->GetYaxis()->SetTitle("Parameter Pull A_{1}");

            _B1pull_v_theta_true = tfs->make<TProfile>(
                "Seed Parameter Pull B_{1} v #theta_{true,fit}  ",
                "Seed Parameter Pull B_{1} v #theta_{true,fit} ", 100, 0,
                3.141529, -10, 10);
            _B1pull_v_theta_true->GetXaxis()->SetTitle("#theta_{true}");
            _B1pull_v_theta_true->GetYaxis()->SetTitle("Parameter Pull B_{1}");

            _A0SeedMCDiff =
                tfs->make<TH1F>("A^{seed}_{0}- A^{MC}_{0} ",
                                "A^{seed}_{0}- A^{MC}_{0}", 100, -100, 100);
            _A0SeedMCDiff->GetXaxis()->SetTitle("#Delta #A_{0}");
            _A0SeedMCDiff->SetStats();

            _A1SeedMCDiff =
                tfs->make<TH1F>("A^{seed}_{1} - A^{MC}_{1}",
                                "A^{seed}_{1} - A^{MC}_{1}", 100, -0.5, 0.5);
            _A1SeedMCDiff->GetXaxis()->SetTitle("#Delta A_{1}");
            _A1SeedMCDiff->SetStats();

            _B0SeedMCDiff =
                tfs->make<TH1F>("B^{seed}_{0} - B^{seed}_{0}",
                                "B^{seed}_{0} - B^{MC}_{0}", 100, -100, 100);
            _B0SeedMCDiff->GetXaxis()->SetTitle("#Delta B^{seed}_{0}");
            _B0SeedMCDiff->SetStats();

            _B1SeedMCDiff =
                tfs->make<TH1F>("B^{seed}_{1} - B^{seed}_{1}",
                                "B^{seed}_{1} - B^{seed}_{1}", 100, -0.5, 0.5);
            _B1SeedMCDiff->GetXaxis()->SetTitle("#Delta B^{seed}_{1}");
            _B1SeedMCDiff->SetStats();

            _A0MinuitMCDiff =
                tfs->make<TH1F>(" A0_{Minuit} - A0_{MC} ",
                                " A0_{Minuit} - A0_{Minuit}", 100, -100, 100);
            _A0MinuitMCDiff->GetXaxis()->SetTitle("#Delta A0 [mm]");
            _A0MinuitMCDiff->SetStats();

            _A1MinuitMCDiff =
                tfs->make<TH1F>(" A1_{Minuit} - A1_{MC} ",
                                " A1_{Minuit} - A1_{MC}", 100, -0.5, 0.5);
            _A1MinuitMCDiff->GetXaxis()->SetTitle("#Delta A1 ");
            _A1MinuitMCDiff->SetStats();

            _B0MinuitMCDiff =
                tfs->make<TH1F>(" B0_{Minuit} - B0_{MC} ",
                                " B0_{Minuit} - B0_{MC}", 100, -100, 100);
            _B0MinuitMCDiff->GetXaxis()->SetTitle("#Delta B0 [mm]");
            _B0MinuitMCDiff->SetStats();

            _B1MinuitMCDiff =
                tfs->make<TH1F>(" B1_{Minuit} - B1_{MC}  ",
                                " B1_{Minuit} - B1_{MC}", 100, -0.5, 0.5);
            _B1MinuitMCDiff->GetXaxis()->SetTitle("#Delta B1 ");
            _B1MinuitMCDiff->SetStats();

            _seedDeltaA0_v_minuitA0 = tfs->make<TH2F>(
                "#Delta A^{seed-MC}_{0} v A^{straw}_{0}", "A^{straw}_{0}", 50,
                -100, 100, 50, -5000, 5000);
            _seedDeltaA0_v_minuitA0->GetXaxis()->SetTitle(
                "#Delta A^{seed-MC}_{0}");
            _seedDeltaA0_v_minuitA0->GetYaxis()->SetTitle("A^{straw}_{0}");

            _seedDeltaA1_v_minuitA1 =
                tfs->make<TH2F>("#Delta A^{seed-MC}_{1} v A^{straw}_{1}",
                                "A^{straw}_{1}", 50, -0.5, 0.5, 50, -5, 5);
            _seedDeltaA1_v_minuitA1->GetXaxis()->SetTitle(
                "#Delta A^{seed-MC}_{1}");
            _seedDeltaA1_v_minuitA1->GetYaxis()->SetTitle("A^{straw}_{1}");

            _seedDeltaB0_v_minuitB0 = tfs->make<TH2F>(
                "#Delta B^{seed-MC}_{0} v B^{straw}_{0}", "B^{straw}_{0}", 50,
                -100, 100, 50, -5000, 5000);
            _seedDeltaB0_v_minuitB0->GetXaxis()->SetTitle(
                "#Delta B^{seed-MC}_{0}");
            _seedDeltaB0_v_minuitB0->GetYaxis()->SetTitle("B^{straw}_{0}");

            _seedDeltaB1_v_minuitB1 =
                tfs->make<TH2F>("#Delta B^{seed-MC}_{1} v B^{straw}_{1}",
                                "B^{straw}_{1}", 50, -0.5, 0.5, 50, -5, 5);
            _seedDeltaB1_v_minuitB1->GetXaxis()->SetTitle(
                "#Delta B^{seed-MC}_{1}");
            _seedDeltaB1_v_minuitB1->GetYaxis()->SetTitle("B^{straw}_{1}");
        }
        _reco_phi_angle =
            tfs->make<TH1F>("#phi Angle of Reconstructred Track",
                            "#phi_{reco} Angle of Tracl ", 20, 0, 3.141529);
        ...
        _seed_a0 = tfs->make<TH1F>("Seed Track Parameter A0 ",
                                   "Seed Track Parameter A0 ", 50, -2000, 2000);
        _seed_a0->GetXaxis()->SetTitle("Seed Track Parameter A0");
        _seed_a0->SetStats();

        _seed_b0 =
            tfs->make<TH1F>("Seed  Track Parameter B0 ",
                            "Seed Track Parameter B0  ", 50, -2000, 2000);
        _seed_b0->GetXaxis()->SetTitle("Seed Track Parameter B0");
        _seed_b0->SetStats();

        _seed_a1 = tfs->make<TH1F>("Seed Track Parameter A1  ",
                                   "Seed vTrack Parameter A1 ", 50, -1, 1);
        _seed_a1->GetXaxis()->SetTitle("Seed Track Parameter A1");
        _seed_a1->SetStats();

        _seed_b1 = tfs->make<TH1F>("Seed Track Parameter B1  ",
                                   "Seed Track Parameter B1  ", 50, -1, 1);
        _seed_b1->GetXaxis()->SetTitle("Seed Track Parameter B1");
        _seed_b1->SetStats();

        _seed_a0XYZ =
            tfs->make<TH1F>("Seed Track Parameter A0 XYZ",
                            "Seed Track Parameter A0 XYZ ", 50, -10000, 10000);
        _seed_a0XYZ->GetXaxis()->SetTitle("Seed Track Parameter A0 XYZ");
        _seed_a0XYZ->SetStats();

        _seed_b0XYZ =
            tfs->make<TH1F>("Seedf Track Parameter B0 XYZ",
                            "Seed Track Parameter B0 XYZ ", 50, -5000, 5000);
        _seed_b0XYZ->GetXaxis()->SetTitle("Seed Track Parameter B0 XYZ");
        _seed_b0XYZ->SetStats();

        _seed_a1XYZ = tfs->make<TH1F>("Seedf Track Parameter A1 XYZ ",
                                      "Seed Track Parameter A1 XYZ", 50, -5, 5);
        _seed_a1XYZ->GetXaxis()->SetTitle("Seed Track Parameter A1 XYZ");
        _seed_a1XYZ->SetStats();

        _seed_b1XYZ =
            tfs->make<TH1F>("Seed Track Parameter B1 XYZ ",
                            "Seed Track Parameter B1 XYZ ", 50, -5, 5);
        _seed_b1XYZ->GetXaxis()->SetTitle("Seed Track Parameter B1");
        _seed_b1XYZ->SetStats();

        _niters = tfs->make<TH1F>(
            "Seed Number of Iterations Until Seed Fit Converged  ",
            "Number of Iterations Until Seed Fit Converged ", 100, 0, 100);
        _niters->GetXaxis()->SetTitle(
            "Seed Number of Iterations Until Seed Fit Converged");
        _niters->SetStats();

        _seedA0_v_seedA1 =
            tfs->make<TH2F>("Seed A_{0} v A_{1}", "Seed A_{0} v A_{1}", 50,
                            -0.2, 0.2, 50, -1500, 1500);
        _seedA0_v_seedA1->GetXaxis()->SetTitle("A_{1}");
        _seedA0_v_seedA1->GetYaxis()->SetTitle("A_{0}");

        _seedB0_v_seedB1 =
            tfs->make<TH2F>("Seed B_{0} v B_{1}", "Seed B_{0} v B_{1}", 50,
                            -0.2, 0.2, 50, -1500, 1500);
        _seedB0_v_seedB1->GetXaxis()->SetTitle("B_{1}");
        _seedB0_v_seedB1->GetYaxis()->SetTitle("B_{0}");

        _seedA0_v_seedB1 =
            tfs->make<TH2F>("Seed A_{0} v B_{1}", "Seed A_{0} v B_{1}", 50,
                            -0.2, 0.2, 50, -1500, 1500);
        _seedA0_v_seedB1->GetXaxis()->SetTitle("B_{1}");
        _seedA0_v_seedB1->GetYaxis()->SetTitle("A_{0");

        _seedA0_v_seedB0 =
            tfs->make<TH2F>("Seed B_{0} v A_{0}", "Seed B_{0} v A_{0}", 50,
                            -1500, 1500, 50, -2000, 2000);
        _seedA0_v_seedB0->GetXaxis()->SetTitle("B_{0}");
        _seedA0_v_seedB0->GetYaxis()->SetTitle("A_{0} ");

        _seedB1_v_seedA1 =
            tfs->make<TH2F>("Seed B_{1} v A_{1}", "Seed B_{1} v A_{1}", 100,
                            -0.2, 0.2, 100, -0.2, 0.2);
        _seedB1_v_seedA1->GetXaxis()->SetTitle("B_{1}");
        _seedB1_v_seedA1->GetYaxis()->SetTitle("A_{1} ");

        _Seed_Cov_A0 =
            tfs->make<TH1F>("#sigma_{A_{0}}", "#sigma_{A_{0}}", 100, 0, 100);
        _Seed_Cov_A0->GetXaxis()->SetTitle("#sigma_{A_{0}}");

        _Seed_Cov_A0A1 =
            tfs->make<TH1F>("Seed #sigma_{A_{0}A_{1}}",
                            "Seed #sigma_{A_{0}A_{1}}", 100, 0, 100);
        _Seed_Cov_A0A1->GetXaxis()->SetTitle("#sigma_{A_{0}A_{1}}");

        _Seed_Cov_A1 = tfs->make<TH1F>("Seed #sigma_{A_{1}}",
                                       "Seed #sigma_{A_{1}}", 100, 0, 0.5);
        _Seed_Cov_A1->GetXaxis()->SetTitle("#sigma_{A_{1}}");

        _Seed_Cov_B0 = tfs->make<TH1F>("Seed #sigma_{B_{0}}",
                                       "Seed #sigma_{B_{0}}", 100, 0, 100);
        _Seed_Cov_B0->GetXaxis()->SetTitle("#sigma_{B_{0}}");

        _Seed_Cov_B1 = tfs->make<TH1F>("Seed #sigma_{B_{1}}",
                                       "Seed #sigma_{B_{1}}", 100, 0, 0.5);
        _Seed_Cov_B1->GetXaxis()->SetTitle("#sigma_{B_{1}}");

        _Seed_Cov_B0B1 =
            tfs->make<TH1F>("Seed #sigma_{B_{0}B_{1}}",
                            "Seed #sigma_{B_{0}B_{1}}", 100, 0, 100);
        _Seed_Cov_B0B1->GetXaxis()->SetTitle("#sigma_{B_{0}B_{1}}");

        _chisq_ndf_plot_init = tfs->make<TH1F>(
            "Seed Init ChiSq/NDF", "Seed Init ChiSq/NDF", 100, 0, 10);
        _chisq_ndf_plot_init->GetXaxis()->SetTitle("Init. #Chi^{2}/N");

        _chisq_ndf_plot_final = tfs->make<TH1F>(
            "Seed Final ChiSq/NDF", "Seed Final ChiSq/NDF", 100, 0, 10);
        _chisq_ndf_plot_final->GetXaxis()->SetTitle("Final #Chi^{2}/N");

        _chisq_ndf_plot_finalX = tfs->make<TH1F>(
            "Seed Final ChiSq/NDF X''", "Seed Final ChiSq/NDF X''", 100, 0, 10);
        _chisq_ndf_plot_finalX->GetXaxis()->SetTitle("Final X '' #Chi^{2}/N");

        _chisq_ndf_plot_finalY = tfs->make<TH1F>(
            "Seed Final ChiSq/NDF Y''", "Seed Final ChiSq/NDF Y''", 100, 0, 10);
        _chisq_ndf_plot_finalY->GetXaxis()->SetTitle("Final Y '' #Chi^{2}/N");

        _chisq_ndf_plot_initX = tfs->make<TH1F>(
            "Seed Init Chi2/NDF  X''", "Seed Init Chi2/NDF X''", 100, 0, 10);
        _chisq_ndf_plot_initX->GetXaxis()->SetTitle("Initial X '' #Chi^{2}/N");

        _chisq_ndf_plot_initY = tfs->make<TH1F>(
            "Seed Init Chi2/NDF Y''", "initial chisq_ndf_plot Y''", 100, 0, 10);
        _chisq_ndf_plot_initY->GetXaxis()->SetTitle("Initial Y '' #Chi^{2}/N");

        _change_chisq_ndf_plot_X =
            tfs->make<TH1F>("Seed Total Change in Chisq/ ndf X''",
                            "Total Change in Chisq/ ndf X''", 50, -10, 20);
        _change_chisq_ndf_plot_X->GetXaxis()->SetTitle(
            "Initial X '' #Chi^{2}/N");

        _change_chisq_ndf_plot_Y =
            tfs->make<TH1F>("Seed Total Change in Chisq/ ndf Y''",
                            "Seed Total Change in Chisq/ ndf Y''", 50, -10, 20);
        _change_chisq_ndf_plot_Y->GetXaxis()->SetTitle(
            "Initial Y '' #Chi^{2}/N");

        _change_chisq_ndf_plot_Total =
            tfs->make<TH1F>("Seed Total Change in #Chi^{2}/ ndf",
                            "Seed Total Change in #Chi^{2}/ ndf", 50, -5, 20);
        _change_chisq_ndf_plot_Total->GetXaxis()->SetTitle(
            "Total Change in #Chi^{2}/ndf");

        _total_residualsX_init =
            tfs->make<TH1F>("Seedf Initial Residuals X'' ",
                            "Initial Residuals X'' ", 50, -1000, 1000);
        _total_residualsX_init->GetXaxis()->SetTitle(
            "Initial Residual X'' [mm]");
        _total_residualsX_init->SetStats();

        _total_residualsY_init =
            tfs->make<TH1F>("Seed Initial Residuals Y''",
                            "Seed Initial Residuals Y'' ", 50, -1000, 1000);
        _total_residualsY_init->GetXaxis()->SetTitle(
            "Initial Residual Y'' [mm]");
        _total_residualsY_init->SetStats();

        _total_pullsX_init = tfs->make<TH1F>("Seed Initial PullSeed  X",
                                             "Initial Pull X''", 200, -50, 50);
        _total_pullsX_init->GetXaxis()->SetTitle("Pull X");
        _total_pullsX_init->SetStats();

        _total_pullsY_init = tfs->make<TH1F>(
            "Seed Initial Pull Y''", "Seed Initial Pull Y''", 200, -50, 50);
        _total_pullsY_init->GetXaxis()->SetTitle("Initial Pull Y");
        _total_pullsY_init->SetStats();

        _total_residualsX_final = tfs->make<TH1F>(
            "Seed Final Residuals X''  ",
            "Seed Final Residuals X''#chi^{2}/dof all  ", 100, -200, 200);
        _total_residualsX_final->GetXaxis()->SetTitle("Residual X'' [mm]");
        _total_residualsX_final->SetStats();

        _total_residualsY_final = tfs->make<TH1F>(
            "Seed Final Residuals Y'' ",
            "Seed Final Residuals Y'' #chi^{2}/dof all ", 100, -200, 200);
        _total_residualsY_final->GetXaxis()->SetTitle(
            "Final Residual Y'' [mm]");
        _total_residualsY_final->SetStats();

        _total_pullsX_final =
            tfs->make<TH1F>("Seed Final Pull X'' #chi^{2}/dof alSeed l ",
                            "Seed Final Pull X''#chi^{2}/dof all ", 50, -2, 2);
        _total_pullsX_final->GetXaxis()->SetTitle("Final Pull X''");

        _total_pullsY_final =
            tfs->make<TH1F>("Seed Final Pull Y''#chi^{2}/dof all ",
                            "Seed Final Pull Y''#chi^{2}/dof all ", 50, -2, 2);
        _total_pullsY_final->GetXaxis()->SetTitle("Final Pull Y''");

        _FinalErrX = tfs->make<TH1F>("Seed Final Errors in X''",
                                     "Seed Final Errors in X'' ", 50, 0, 100);
        _FinalErrX->GetXaxis()->SetTitle("#sigma_{X''} [mm]");
        _FinalErrX->SetStats();

        _FinalErrY = tfs->make<TH1F>("Seed Final Errors in Y''",
                                     "Seed Final Errors in Y'' ", 50, 0, 100);
        _FinalErrY->GetXaxis()->SetTitle("#sigma_{Y''} [mm]");
        _FinalErrY->SetStats();

        _InitErrX = tfs->make<TH1F>("Seed Initial Errors in X''",
                                    "Seed Initial Errors in X'' ", 50, 0, 100);
        _InitErrX->GetXaxis()->SetTitle("#sigma_{X''} [mm]");
        _InitErrX->SetStats();

        _InitErrY = tfs->make<TH1F>("Seed Initial Errors in Y''",
                                    "Seed Initial Errors in Y'' ", 50, 0, 100);
        _InitErrY->GetXaxis()->SetTitle("#sigma_{Y''} [mm]");
        _InitErrY->SetStats();

        _FinalErrTot = tfs->make<TH1F>("Seed Final Errors Total",
                                       "Seed Final Errors Total ", 50, 0, 100);
        _FinalErrTot->GetXaxis()->SetTitle("#sigma_{Y''} [mm]");
        _FinalErrTot->SetStats();

        _InitErrTot = tfs->make<TH1F>("Seed Initial Errors Total",
                                      "Seed Initial Errors Total ", 50, 0, 100);
        _InitErrTot->GetXaxis()->SetTitle("#sigma_{X''} [mm]");
        _InitErrTot->SetStats();

        //------------------------Straw Fit-------------------------------//

        _total_residualsX_Minuit =
            tfs->make<TH1F>("Minuit Residuals X''  ",
                            "Minuit Final Residuals X'' ", 100, -200, 200);
        _total_residualsX_Minuit->GetXaxis()->SetTitle("Minuit Res. X'' [mm]");
        _total_residualsX_Minuit->SetStats();

        _total_residualsY_Minuit = tfs->make<TH1F>(
            "Minuit Residuals Y'' ", "Minuit Residuals Y''  ", 100, -200, 200);
        _total_residualsY_Minuit->GetXaxis()->SetTitle("Minuit Res. Y'' [mm]");
        _total_residualsY_Minuit->SetStats();

        _FullFitEndDOCAs =
            tfs->make<TH1F>("Final DOCAs ", "Final DOCAs", 50, 0, 10);
        _FullFitEndDOCAs->GetXaxis()->SetTitle("Final DOCAs [mm]");
        _FullFitEndDOCAs->SetStats();

        _GaussianEndDOCAs =
            tfs->make<TH1F>("Gaussian DOCAs ", "Gaussian DOCAs", 50, 0, 10);
        _GaussianEndDOCAs->GetXaxis()->SetTitle("Gaussian DOCAs [mm]");
        _GaussianEndDOCAs->SetStats();

        _StartDOCAs =
            tfs->make<TH1F>("Initial DOCAs ", "InitialDOCAs", 50, 0, 10);
        _StartDOCAs->GetXaxis()->SetTitle("Initial DOCAs [mm]");
        _StartDOCAs->SetStats();

        _FullFitEndTimeResiduals =
            tfs->make<TH1F>("Final Time Res ", "Final Time Resid", 50, 0, 500);
        _FullFitEndTimeResiduals->GetXaxis()->SetTitle("Final Time Res [ns]");
        _FullFitEndTimeResiduals->SetStats();

        _GaussianEndTimeResiduals = tfs->make<TH1F>(
            "Gaussian Time Res ", "Gaussian Time Resid", 50, 0, 500);
        _GaussianEndTimeResiduals->GetXaxis()->SetTitle(
            "Gaussian Time Res [ns]");
        _GaussianEndTimeResiduals->SetStats();

        _StartTimeResiduals =
            tfs->make<TH1F>("Start Time Res ", "Start Time Resid", 50, 0, 500);
        _StartTimeResiduals->GetXaxis()->SetTitle("Start Time Res [ns]");
        _StartTimeResiduals->SetStats();

        _A0MinuitFitDiff = tfs->make<TH1F>(" A0 Minuit Diff ",
                                           " A0 Minuit Diff", 100, -100, 100);
        _A0MinuitFitDiff->GetXaxis()->SetTitle("#Delta A0 [mm]");
        _A0MinuitFitDiff->SetStats();

        _A1MinuitFitDiff =
            tfs->make<TH1F>(" A1 Minuit Diff ", " A1 Minuit Diff", 100, -3, 3);
        _A1MinuitFitDiff->GetXaxis()->SetTitle("#Delta A1 ");
        _A1MinuitFitDiff->SetStats();

        _B0MinuitFitDiff = tfs->make<TH1F>(" B0 Minuit Diff ",
                                           " B0 Minuit Diff", 100, -100, 100);
        _B0MinuitFitDiff->GetXaxis()->SetTitle("#Delta B0 [mm] ");
        _B0MinuitFitDiff->SetStats();

        _B1MinuitFitDiff =
            tfs->make<TH1F>(" B1 Minuit Diff ", " B1 Minuit Diff", 100, -3, 3);
        _B1MinuitFitDiff->GetXaxis()->SetTitle("#Delta B1 ");
        _B1MinuitFitDiff->SetStats();

        _minuit_pullsX_final =
            tfs->make<TH1F>("Miuit Pull X'' ", "Minuit Pull X'' ", 50, -2, 2);
        _minuit_pullsX_final->GetXaxis()->SetTitle("Minuit Pull X''");

        _minuit_pullsY_final =
            tfs->make<TH1F>("Minuit Pull Y'' ", "Minuit Pull Y'' ", 50, -2, 2);
        ...
        _A0Minuit = tfs->make<TH1F>(" A0_{Minuit} ", " A0_{Minuit} ", 50,
                                    -10000, 10000);
        _A0Minuit->GetXaxis()->SetTitle(" [mm]");
        _A0Minuit->SetStats();

        _A1Minuit =
            tfs->make<TH1F>(" A1_{Minuit}  ", " A1_{Minuit}", 50, -5, 5);
        _A1Minuit->GetXaxis()->SetTitle("A1 ");
        _A1Minuit->SetStats();

        _B0Minuit =
            tfs->make<TH1F>(" B0_{Minuit}", " B0_{Minuit} ", 50, -5000, 5000);
        _B0Minuit->GetXaxis()->SetTitle(" [mm]");
        _B0Minuit->SetStats();

        _B1Minuit =
            tfs->make<TH1F>(" B1_{Minuit}  ", " B1_{Minuit} ", 50, -5, 5);
...
void CosmicAnalyzer::analyze(const art::Event &event) {
    ...
    for (size_t ist = 0; ist < _coscol->size(); ++ist) {
        ...
        if (st.converged == false or st.minuit_converged == false) {
            continue;
        }
        ...
        _A0MinuitFitDiff->Fill(st.MinuitFitParams.A0 -
                               st.FitEquationXYZ.Pos.X());
        _A1MinuitFitDiff->Fill(st.MinuitFitParams.A1 -
                               st.FitEquationXYZ.Dir.X());
        _B0MinuitFitDiff->Fill(st.MinuitFitParams.B0 -
                               st.FitEquationXYZ.Pos.Y());
        _B1MinuitFitDiff->Fill(st.MinuitFitParams.B1 -
                               st.FitEquationXYZ.Dir.Y());
        ...
        if (_mcdiag) {
            ...
            if (st.MinuitFitParams.Covarience.sigA0 != 0) {
                _A0MinuitMCDiff->Fill(
                    (trueinfo.TrueFitEquation.Pos.X() -
                     st.MinuitFitParams
                         .A0)); ///(st.MinuitFitParams.Covarience.sigA0));
            }
            if (st.MinuitFitParams.Covarience.sigA1 != 0) {
                _A1MinuitMCDiff->Fill(
                    (trueinfo.TrueFitEquation.Dir.X() -
                     st.MinuitFitParams
                         .A1)); ///(st.MinuitFitParams.Covarience.sigA1));
            }
            if (st.MinuitFitParams.Covarience.sigB0 != 0) {
                _B0MinuitMCDiff->Fill(
                    (trueinfo.TrueFitEquation.Pos.Y() -
                     st.MinuitFitParams
                         .B0)); ///(st.MinuitFitParams.Covarience.sigB0));
            }
            if (st.MinuitFitParams.Covarience.sigB1 != 0) {
                _B1MinuitMCDiff->Fill(
                    (trueinfo.TrueFitEquation.Dir.Y() -
                     st.MinuitFitParams
                         .B1)); //(st.MinuitFitParams.Covarience.sigB1));
            }

            _A0SeedMCDiff->Fill(trueinfo.TrueFitEquation.Pos.X() -
                                st.FitEquationXYZ.Pos.X());
            _A1SeedMCDiff->Fill(trueinfo.TrueFitEquation.Dir.X() -
                                st.FitEquationXYZ.Dir.X());
            _B0SeedMCDiff->Fill(trueinfo.TrueFitEquation.Pos.Y() -
                                st.FitEquationXYZ.Pos.Y());
            _B1SeedMCDiff->Fill(trueinfo.TrueFitEquation.Dir.Y() -
                                st.FitEquationXYZ.Dir.Y());

            _seedDeltaA0_v_minuitA0->Fill(trueinfo.TrueFitEquation.Pos.X() -
                                              st.FitEquationXYZ.Pos.X(),
                                          st.MinuitFitParams.A0);
            _seedDeltaB0_v_minuitB0->Fill(trueinfo.TrueFitEquation.Pos.Y() -
                                              st.FitEquationXYZ.Pos.Y(),
                                          st.MinuitFitParams.B0);
            _seedDeltaA1_v_minuitA1->Fill(trueinfo.TrueFitEquation.Dir.X() -
                                              st.FitEquationXYZ.Dir.X(),
                                          st.MinuitFitParams.A1);
            _seedDeltaB1_v_minuitB1->Fill(trueinfo.TrueFitEquation.Dir.Y() -
                                              st.FitEquationXYZ.Dir.Y(),
        ...
        _change_chisq_ndf_plot_Total->Fill(st.Diag.InitialChiTot -
                                           st.Diag.FinalChiTot);

        for (size_t i = 0; i < sts._straw_chits.size(); i++) {
            ComboHit const &chit = sts._straw_chits[i];

            double StartDOCA = DriftFitUtils::GetTestDOCA(
                chit, st.FitParams.A0, st.FitParams.A1, st.FitParams.B0,
                st.FitParams.B1, tracker);
            _StartDOCAs->Fill(StartDOCA);
            // double StartTOCA = DriftFitUtils::TimeResidual(StartDOCA, srep,
            // sts._t0.t0(), chit); _FullFitEndTimeResiduals->Fill(TOCA);
            ///_StartTimeResiduals->Fill(st.DriftDiag.StartTimeResiduals[i]);

            double DOCA = DriftFitUtils::GetTestDOCA(
                chit, st.MinuitFitParams.A0, st.MinuitFitParams.A1,
                st.MinuitFitParams.B0, st.MinuitFitParams.B1, tracker);
            _FullFitEndDOCAs->Fill(DOCA);
            // double TOCA = DriftFitUtils::TimeResidual(DOCA, srep,
            // sts._t0.t0(), chit); _FullFitEndTimeResiduals->Fill(TOCA);
            int RecoAmbig = DriftFitUtils::GetAmbig(
                chit, st.MinuitFitParams.A0, st.MinuitFitParams.A1,
                st.MinuitFitParams.B0, st.MinuitFitParams.B1, tracker);
            _FullFitEndDOCAs->Fill(DOCA);
            ...
            if (_mcdiag) {
                ...
                if (DriftFitUtils::GetTestDOCA(
                        chit, trueinfo.TrueFitEquation.Pos.X(),
                        trueinfo.TrueFitEquation.Dir.X(),
                        trueinfo.TrueFitEquation.Pos.Y(),
                        trueinfo.TrueFitEquation.Dir.Y(), tracker) < 2.5 and
                    DOCA < 2.5 and
                    abs(trueinfo.TrueFitEquation.Pos.X()) < 5000 and
                    abs(trueinfo.TrueFitEquation.Pos.Y()) < 5000 and
                    abs(trueinfo.TrueFitEquation.Dir.X()) < 5 and
                    abs(trueinfo.TrueFitEquation.Dir.Y()) < 5) {

                    outputfile
                        << DriftFitUtils::GetTestDOCA(
                               chit, trueinfo.TrueFitEquation.Pos.X(),
                               trueinfo.TrueFitEquation.Dir.X(),
                               trueinfo.TrueFitEquation.Pos.Y(),
                               trueinfo.TrueFitEquation.Dir.Y(), tracker)
                        << "," << DOCA << ","
                        << RecoAmbig /
                               DriftFitUtils::GetAmbig(
                                   chit, trueinfo.TrueFitEquation.Pos.X(),
                                   trueinfo.TrueFitEquation.Dir.X(),
                                   trueinfo.TrueFitEquation.Pos.Y(),
                                   trueinfo.TrueFitEquation.Dir.Y(), tracker)
                        << "," << trueinfo.TrueTheta << ","
                        << st.MinuitFitParams.A0 << "," << st.MinuitFitParams.A1
                        << "," << st.MinuitFitParams.B0 << ","
                        << st.MinuitFitParams.B1 << ","
                        << trueinfo.TrueFitEquation.Pos.X() << ","
                        << trueinfo.TrueFitEquation.Dir.X() << ","
                        << trueinfo.TrueFitEquation.Pos.Y() << ","
                        << trueinfo.TrueFitEquation.Dir.Y() << endl;
                }
            }
        }
        ...
        for (auto const &tseed : *_coscol) {
            ...
            _StraightTrackConverged =
                status.hasAllProperties(TrkFitFlag::helixConverged);
            _StraightTrackInit =
                status.hasAllProperties(TrkFitFlag::circleInit);
        }
        ...
        //----------------Get panels/planes/stations per
        //track:------------------//
    ...
    if (_mcdiag) {
        cout << "true + " << _AMBIG->GetBinContent(2, 2) / _AMBIG->Integral()
             << endl;
        cout << "true - " << _AMBIG->GetBinContent(1, 1) / _AMBIG->Integral()
             << endl;
        cout << "false + " << _AMBIG->GetBinContent(2, 1) / _AMBIG->Integral()
             << endl;
        cout << "false - " << _AMBIG->GetBinContent(1, 2) / _AMBIG->Integral()
             << endl;
    }
}

void CosmicAnalyzer::endJob() {
    if (_diag and _mcdiag)
        outputfile.close();
}

CosmicTrackMCInfo
CosmicAnalyzer::FitMC(const StrawDigiMCCollection *&_mcdigis) {
    ...
    XYZVec pos0(spmcp0->position().x(), spmcp0->position().y(),
                spmcp0->position().z());
    XYZVec dir0(spmcp0->momentum().x(), spmcp0->momentum().y(),
                spmcp0->momentum().z());
    ...
    for (size_t ich = 0; ich < _mcdigis->size(); ++ich) {
        ...
        XYZVec posN(spmcp->position().x(), spmcp->position().y(),
                    spmcp->position().z());
    ...
    TrackParams RawTrueParams(S.GetAlphaX()[0][0], S.GetAlphaX()[1][0],
                              S.GetAlphaY()[0][0], S.GetAlphaY()[1][0]);
...
CosmicTrackMCInfo CosmicAnalyzer::FillDriftMC(ComboHit const &chit,
                                              double RecoAmbig,
                                              CosmicTrackMCInfo info,
                                              const Tracker *tracker) {

    double true_doca = DriftFitUtils::GetTestDOCA(
        chit, info.TrueFitEquation.Pos.X(), info.TrueFitEquation.Dir.X(),
        info.TrueFitEquation.Pos.Y(), info.TrueFitEquation.Dir.Y(), tracker);
    double trueambig = DriftFitUtils::GetAmbig(
        chit, info.TrueFitEquation.Pos.X(), info.TrueFitEquation.Dir.X(),
        info.TrueFitEquation.Pos.Y(), info.TrueFitEquation.Dir.Y(), tracker);
    // double true_time_residual = DriftFitUtils::TimeResidual(true_doca, _srep,
    // sts._t0.t0(), chit);
...
bool CosmicAnalyzer::findData(const art::Event &evt) {
    ...
    return _chcol != 0 && _tccol != 0 && _coscol != 0 &&
           (_mcdigis != 0 || !_mcdiag);
}
