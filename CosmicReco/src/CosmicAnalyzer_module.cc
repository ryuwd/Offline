...
public:
  struct Config {
    ...
    fhicl::Atom<bool> mcdiag{Name("mcdiag"), Comment("set on for MC info"),
                             true};
    fhicl::Atom<art::InputTag> chtag{Name("ComboHitCollection"),
    ...
    fhicl::Atom<art::InputTag> mcdigistag{Name("StrawDigiMCCollection"),
...
private:
  ...
  CosmicTrackMCInfo FillDriftMC(ComboHit const &chit, double reco_ambig,
                                CosmicTrackMCInfo info, const Tracker *tracker);
...
void CosmicAnalyzer::beginJob() {
  ...
  if (_diag > 0) {
    ...
    _cosmic_analysis->Branch("StraightTrackOK", &_StraightTrackOK,
                             "StraightTrackOK/B");
    _cosmic_analysis->Branch("StraightTrackConverged", &_StraightTrackConverged,
    ...
    if (_mcdiag) {
      ...
      _truea0XYZ =
          tfs->make<TH1F>("True Track Parameter A0 XYZ",
                          "True Track Parameter A0 XYZ ", 50, -10000, 10000);
      _truea0XYZ->GetXaxis()->SetTitle("Track Parameter A0 XYZ");
      _truea0XYZ->SetStats();

      _trueb0XYZ =
          tfs->make<TH1F>("True Track Parameter B0 XYZ",
                          "True Track Parameter B0 XYZ ", 50, -5000, 5000);
      ...
      _mc_phi_angle = tfs->make<TH1F>("#phi_{true, fit}", "#phi_{true, fit}",
                                      100, -3.141529, 3.141529);
      _mc_phi_angle->GetXaxis()->SetTitle("#phi_{true, fit} [rad]");

      _mc_theta_angle = tfs->make<TH1F>("#theta_{true, fit}",
                                        "#theta_{true, fit}", 20, 0, 3.141529);
      _mc_theta_angle->GetXaxis()->SetTitle("#theta_{true, fit} [rad]");

      _AMBIG =
          tfs->make<TH2F>("True v Reco", "True v Reco", 2, -2, 2, 2, -2, 2);
      ...
      _TrueTimeResiduals =
          tfs->make<TH1F>("True Time Res ", "True Time Resid", 50, 0, 500);
      _TrueTimeResiduals->GetXaxis()->SetTitle("True Time Res [ns]");
      _TrueTimeResiduals->SetStats();

      _A0pull_v_theta_true =
          tfs->make<TProfile>("Seed Parameter Pull A_{0} v #theta_{true,fit}  ",
                              "Seed Parameter Pull A_{0} v #theta_{true,fit} ",
                              100, 0, 3.141529, -200, 200);
      _A0pull_v_theta_true->GetXaxis()->SetTitle("#theta_{true}");
      _A0pull_v_theta_true->GetYaxis()->SetTitle("Parameter Pull A_{0}");

      _B0pull_v_theta_true =
          tfs->make<TProfile>("Seed Parameter Pull B_{0} v #theta_{true,fit}  ",
      ...
      _A1pull_v_theta_true =
          tfs->make<TProfile>("Seed Parameter Pull A_{1} v #theta_{true,fit}  ",
                              "Seed Parameter Pull A_{1} v #theta_{true,fit} ",
                              100, 0, 3.141529, -10, 10);
      _A1pull_v_theta_true->GetXaxis()->SetTitle("#theta_{true}");
      _A1pull_v_theta_true->GetYaxis()->SetTitle("Parameter Pull A_{1}");

      _B1pull_v_theta_true =
          tfs->make<TProfile>("Seed Parameter Pull B_{1} v #theta_{true,fit}  ",
      ...
      _A1MinuitMCDiff = tfs->make<TH1F>(
          " A1_{Minuit} - A1_{MC} ", " A1_{Minuit} - A1_{MC}", 100, -0.5, 0.5);
      _A1MinuitMCDiff->GetXaxis()->SetTitle("#Delta A1 ");
      _A1MinuitMCDiff->SetStats();

      _B0MinuitMCDiff = tfs->make<TH1F>(
          " B0_{Minuit} - B0_{MC} ", " B0_{Minuit} - B0_{MC}", 100, -100, 100);
      _B0MinuitMCDiff->GetXaxis()->SetTitle("#Delta B0 [mm]");
      _B0MinuitMCDiff->SetStats();

      _B1MinuitMCDiff = tfs->make<TH1F>(
          " B1_{Minuit} - B1_{MC}  ", " B1_{Minuit} - B1_{MC}", 100, -0.5, 0.5);
      _B1MinuitMCDiff->GetXaxis()->SetTitle("#Delta B1 ");
      _B1MinuitMCDiff->SetStats();

      _seedDeltaA0_v_minuitA0 =
          tfs->make<TH2F>("#Delta A^{seed-MC}_{0} v A^{straw}_{0}",
                          "A^{straw}_{0}", 50, -100, 100, 50, -5000, 5000);
      ...
      _seedDeltaB0_v_minuitB0 =
          tfs->make<TH2F>("#Delta B^{seed-MC}_{0} v B^{straw}_{0}",
                          "B^{straw}_{0}", 50, -100, 100, 50, -5000, 5000);
    ...
    _Seed_Cov_A1 = tfs->make<TH1F>("Seed #sigma_{A_{1}}", "Seed #sigma_{A_{1}}",
                                   100, 0, 0.5);
    _Seed_Cov_A1->GetXaxis()->SetTitle("#sigma_{A_{1}}");

    _Seed_Cov_B0 = tfs->make<TH1F>("Seed #sigma_{B_{0}}", "Seed #sigma_{B_{0}}",
                                   100, 0, 100);
    _Seed_Cov_B0->GetXaxis()->SetTitle("#sigma_{B_{0}}");

    _Seed_Cov_B1 = tfs->make<TH1F>("Seed #sigma_{B_{1}}", "Seed #sigma_{B_{1}}",
                                   100, 0, 0.5);
    ...
    _chisq_ndf_plot_init = tfs->make<TH1F>("Seed Init ChiSq/NDF",
                                           "Seed Init ChiSq/NDF", 100, 0, 10);
    _chisq_ndf_plot_init->GetXaxis()->SetTitle("Init. #Chi^{2}/N");

    _chisq_ndf_plot_final = tfs->make<TH1F>("Seed Final ChiSq/NDF",
                                            "Seed Final ChiSq/NDF", 100, 0, 10);
    ...
    _total_pullsY_init = tfs->make<TH1F>("Seed Initial Pull Y''",
                                         "Seed Initial Pull Y''", 200, -50, 50);
    ...
    _total_residualsX_Minuit =
        tfs->make<TH1F>("Minuit Residuals X''  ", "Minuit Final Residuals X'' ",
                        100, -200, 200);
    ...
    _A0MinuitFitDiff =
        tfs->make<TH1F>(" A0 Minuit Diff ", " A0 Minuit Diff", 100, -100, 100);
    ...
    _B0MinuitFitDiff =
        tfs->make<TH1F>(" B0 Minuit Diff ", " B0 Minuit Diff", 100, -100, 100);
    ...
    _A0Minuit =
        tfs->make<TH1F>(" A0_{Minuit} ", " A0_{Minuit} ", 50, -10000, 10000);
...
void CosmicAnalyzer::analyze(const art::Event &event) {
  ...
  for (size_t ist = 0; ist < _coscol->size(); ++ist) {
    ...
    for (size_t i = 0; i < sts._straw_chits.size(); i++) {
      ...
      double StartDOCA =
          DriftFitUtils::GetTestDOCA(chit, st.FitParams.A0, st.FitParams.A1,
                                     st.FitParams.B0, st.FitParams.B1, tracker);
