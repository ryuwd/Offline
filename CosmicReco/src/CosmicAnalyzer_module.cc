...
class CosmicAnalyzer : public art::EDAnalyzer
{
  ...
  private:
    ...
    CosmicTrackMCInfo FillDriftMC(ComboHit const& chit, double reco_ambig, CosmicTrackMCInfo info,
                                  const Tracker* tracker);
    bool findData(const art::Event& evt);
};

CosmicAnalyzer::CosmicAnalyzer(const Parameters& conf)
    : art::EDAnalyzer(conf), _diag(conf().diag()), _mcdiag(conf().mcdiag()), _chtag(conf().chtag()),
      _tctag(conf().tctag()), _costag(conf().costag()), _mcdigistag(conf().mcdigistag()), _toff(conf().toff())
{
...
void CosmicAnalyzer::beginJob()
{
    ...
    if (_diag > 0) {
        ...
        if (_mcdiag) {
            ...
            _truea0XYZ =
                tfs->make<TH1F>("True Track Parameter A0 XYZ", "True Track Parameter A0 XYZ ", 50, -10000, 10000);
            _truea0XYZ->GetXaxis()->SetTitle("Track Parameter A0 XYZ");
            _truea0XYZ->SetStats();

            _trueb0XYZ =
                tfs->make<TH1F>("True Track Parameter B0 XYZ", "True Track Parameter B0 XYZ ", 50, -5000, 5000);
            ...
            _A0pull_v_theta_true =
                tfs->make<TProfile>("Seed Parameter Pull A_{0} v #theta_{true,fit}  ",
                                    "Seed Parameter Pull A_{0} v #theta_{true,fit} ", 100, 0, 3.141529, -200, 200);
            ...
            _B0pull_v_theta_true =
                tfs->make<TProfile>("Seed Parameter Pull B_{0} v #theta_{true,fit}  ",
                                    "Seed Parameter Pull B_{0} v #theta_{true,fit} ", 100, 0, 3.141529, -200, 200);
            ...
            _A1pull_v_theta_true =
                tfs->make<TProfile>("Seed Parameter Pull A_{1} v #theta_{true,fit}  ",
                                    "Seed Parameter Pull A_{1} v #theta_{true,fit} ", 100, 0, 3.141529, -10, 10);
            ...
            _B1pull_v_theta_true =
                tfs->make<TProfile>("Seed Parameter Pull B_{1} v #theta_{true,fit}  ",
                                    "Seed Parameter Pull B_{1} v #theta_{true,fit} ", 100, 0, 3.141529, -10, 10);
            ...
            _B1SeedMCDiff =
                tfs->make<TH1F>("B^{seed}_{1} - B^{seed}_{1}", "B^{seed}_{1} - B^{seed}_{1}", 100, -0.5, 0.5);
            ...
            _seedDeltaA0_v_minuitA0 = tfs->make<TH2F>("#Delta A^{seed-MC}_{0} v A^{straw}_{0}", "A^{straw}_{0}", 50,
            ...
            _seedDeltaA1_v_minuitA1 =
                tfs->make<TH2F>("#Delta A^{seed-MC}_{1} v A^{straw}_{1}", "A^{straw}_{1}", 50, -0.5, 0.5, 50, -5, 5);
            _seedDeltaA1_v_minuitA1->GetXaxis()->SetTitle("#Delta A^{seed-MC}_{1}");
            _seedDeltaA1_v_minuitA1->GetYaxis()->SetTitle("A^{straw}_{1}");

            _seedDeltaB0_v_minuitB0 = tfs->make<TH2F>("#Delta B^{seed-MC}_{0} v B^{straw}_{0}", "B^{straw}_{0}", 50,
                                                      -100, 100, 50, -5000, 5000);
            ...
            _seedDeltaB1_v_minuitB1 =
                tfs->make<TH2F>("#Delta B^{seed-MC}_{1} v B^{straw}_{1}", "B^{straw}_{1}", 50, -0.5, 0.5, 50, -5, 5);
            _seedDeltaB1_v_minuitB1->GetXaxis()->SetTitle("#Delta B^{seed-MC}_{1}");
            _seedDeltaB1_v_minuitB1->GetYaxis()->SetTitle("B^{straw}_{1}");
        }
        _reco_phi_angle =
            tfs->make<TH1F>("#phi Angle of Reconstructred Track", "#phi_{reco} Angle of Tracl ", 20, 0, 3.141529);
        ...
        _niters = tfs->make<TH1F>("Seed Number of Iterations Until Seed Fit Converged  ",
        ...
        _seedA0_v_seedB0 =
            tfs->make<TH2F>("Seed B_{0} v A_{0}", "Seed B_{0} v A_{0}", 50, -1500, 1500, 50, -2000, 2000);
        ...
        _change_chisq_ndf_plot_X =
            tfs->make<TH1F>("Seed Total Change in Chisq/ ndf X''", "Total Change in Chisq/ ndf X''", 50, -10, 20);
        _change_chisq_ndf_plot_X->GetXaxis()->SetTitle("Initial X '' #Chi^{2}/N");

        _change_chisq_ndf_plot_Y =
            tfs->make<TH1F>("Seed Total Change in Chisq/ ndf Y''", "Seed Total Change in Chisq/ ndf Y''", 50, -10, 20);
        _change_chisq_ndf_plot_Y->GetXaxis()->SetTitle("Initial Y '' #Chi^{2}/N");

        _change_chisq_ndf_plot_Total =
            tfs->make<TH1F>("Seed Total Change in #Chi^{2}/ ndf", "Seed Total Change in #Chi^{2}/ ndf", 50, -5, 20);
        _change_chisq_ndf_plot_Total->GetXaxis()->SetTitle("Total Change in #Chi^{2}/ndf");

        _total_residualsX_init =
            tfs->make<TH1F>("Seedf Initial Residuals X'' ", "Initial Residuals X'' ", 50, -1000, 1000);
        _total_residualsX_init->GetXaxis()->SetTitle("Initial Residual X'' [mm]");
        _total_residualsX_init->SetStats();

        _total_residualsY_init =
            tfs->make<TH1F>("Seed Initial Residuals Y''", "Seed Initial Residuals Y'' ", 50, -1000, 1000);
        ...
        _total_residualsX_final =
            tfs->make<TH1F>("Seed Final Residuals X''  ", "Seed Final Residuals X''#chi^{2}/dof all  ", 100, -200, 200);
        _total_residualsX_final->GetXaxis()->SetTitle("Residual X'' [mm]");
        _total_residualsX_final->SetStats();

        _total_residualsY_final =
            tfs->make<TH1F>("Seed Final Residuals Y'' ", "Seed Final Residuals Y'' #chi^{2}/dof all ", 100, -200, 200);
        ...
        _total_pullsY_final =
            tfs->make<TH1F>("Seed Final Pull Y''#chi^{2}/dof all ", "Seed Final Pull Y''#chi^{2}/dof all ", 50, -2, 2);
        ...
        _total_residualsX_Minuit =
            tfs->make<TH1F>("Minuit Residuals X''  ", "Minuit Final Residuals X'' ", 100, -200, 200);
...
void CosmicAnalyzer::analyze(const art::Event& event)
{
    ...
    for (size_t ist = 0; ist < _coscol->size(); ++ist) {
        ...
        for (size_t i = 0; i < sts._straw_chits.size(); i++) {
            ...
            double StartDOCA = DriftFitUtils::GetTestDOCA(chit, st.FitParams.A0, st.FitParams.A1, st.FitParams.B0,
            ...
            double DOCA = DriftFitUtils::GetTestDOCA(chit, st.MinuitFitParams.A0, st.MinuitFitParams.A1,
            ...
            // sts._t0.t0(), chit); _FullFitEndTimeResiduals->Fill(TOCA);
            int RecoAmbig = DriftFitUtils::GetAmbig(chit, st.MinuitFitParams.A0, st.MinuitFitParams.A1,
                                                    st.MinuitFitParams.B0, st.MinuitFitParams.B1, tracker);
            _FullFitEndDOCAs->Fill(DOCA);
            ...
            if (_mcdiag) {
                ...
                if (DriftFitUtils::GetTestDOCA(chit, trueinfo.TrueFitEquation.Pos.X(), trueinfo.TrueFitEquation.Dir.X(),
                                               trueinfo.TrueFitEquation.Pos.Y(), trueinfo.TrueFitEquation.Dir.Y(),
                                               tracker) < 2.5 and
                    DOCA < 2.5 and abs(trueinfo.TrueFitEquation.Pos.X()) < 5000 and
                    abs(trueinfo.TrueFitEquation.Pos.Y()) < 5000 and abs(trueinfo.TrueFitEquation.Dir.X()) < 5 and
                    abs(trueinfo.TrueFitEquation.Dir.Y()) < 5) {

                    outputfile << DriftFitUtils::GetTestDOCA(
                                      chit, trueinfo.TrueFitEquation.Pos.X(), trueinfo.TrueFitEquation.Dir.X(),
                                      trueinfo.TrueFitEquation.Pos.Y(), trueinfo.TrueFitEquation.Dir.Y(), tracker)
                               << "," << DOCA << ","
                               << RecoAmbig / DriftFitUtils::GetAmbig(chit, trueinfo.TrueFitEquation.Pos.X(),
                                                                      trueinfo.TrueFitEquation.Dir.X(),
                                                                      trueinfo.TrueFitEquation.Pos.Y(),
                                                                      trueinfo.TrueFitEquation.Dir.Y(), tracker)
                               << "," << trueinfo.TrueTheta << "," << st.MinuitFitParams.A0 << ","
                               << st.MinuitFitParams.A1 << "," << st.MinuitFitParams.B0 << "," << st.MinuitFitParams.B1
                               << "," << trueinfo.TrueFitEquation.Pos.X() << "," << trueinfo.TrueFitEquation.Dir.X()
                               << "," << trueinfo.TrueFitEquation.Pos.Y() << "," << trueinfo.TrueFitEquation.Dir.Y()
                               << endl;
                }
            }
...
CosmicTrackMCInfo CosmicAnalyzer::FillDriftMC(ComboHit const& chit, double RecoAmbig, CosmicTrackMCInfo info,
                                              const Tracker* tracker)
{

    double true_doca = DriftFitUtils::GetTestDOCA(chit, info.TrueFitEquation.Pos.X(), info.TrueFitEquation.Dir.X(),
                                                  info.TrueFitEquation.Pos.Y(), info.TrueFitEquation.Dir.Y(), tracker);
    double trueambig = DriftFitUtils::GetAmbig(chit, info.TrueFitEquation.Pos.X(), info.TrueFitEquation.Dir.X(),
                                               info.TrueFitEquation.Pos.Y(), info.TrueFitEquation.Dir.Y(), tracker);
