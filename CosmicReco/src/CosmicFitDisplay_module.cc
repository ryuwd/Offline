...
class CosmicFitDisplay : public art::EDAnalyzer
{
  public:
    struct Config {
        ...
        fhicl::Atom<art::InputTag> sttag{Name("CosmicTrackSeedCollection"),
...
CosmicFitDisplay::CosmicFitDisplay(const Parameters& conf)
    : art::EDAnalyzer(conf), _mcdiag(conf().mcdiag()), _chtag(conf().chtag()), _tctag(conf().tctag()),
      _sttag(conf().sttag()), doDisplay_(conf().doDisplay()), clickToAdvance_(conf().clickToAdvance())
{
}
...
void CosmicFitDisplay::plot2dDriftCompare(const art::Event& event)
{
    ...
    std::vector<double> xseed, yseed, zseed, xdrift, ydrift, zdrift, a0seed, a1seed, b0seed, b1seed, a0drift, a1drift,
        b0drift, b1drift;
    ...
    for (size_t i = 0; i < _ncosmics; i++) {
        ...
        if (track._track.converged == false) {
            continue;
        }
        if (track._track.minuit_converged == false) {
            continue;
        }
        ...
        if (isnan(track._track.FitParams.A0) == true && isnan(track._track.FitParams.A1) == true &&
            isnan(track._track.FitParams.B0) == true && isnan(track._track.FitParams.B1) == true)
            continue;
        ...
        if (doDisplay_) {
            ...
            std::cout << "Run: " << event.id().run() << "  Subrun: " << event.id().subRun()
...
void CosmicFitDisplay::plot2d(const art::Event& event)
{
    ...
    std::vector<double> initpullsx, initpullsy, pullsx, pullsy, x, y, z, rawx, rawy, rawz, xinit, yinit, zinit, out_x,
        out_y, out_z, a0, a1, b0, b1, a0init, a1init, b0init, b1init, chi_dof_XDoublePrimeZPrime,
        chi_dof_YDoublePrimeZPrime, initchi_dof_YDoublePrimeZPrime, initchi_dof_XDoublePrimeZPrime;
...
void CosmicFitDisplay::plot3dXYZ(const art::Event& event)
{
    ...
    if (xprimes.size() > 0) {
        ...
        if (doDisplay_) {
            ...
            TH3D* xyzplot =
                new TH3D("XYZ", "XYZ", 1, -plotLimits, plotLimits, 1, -plotLimits, plotLimits, 1, -zlimit, zlimit);
...
{ // local coords
    ...
    if (xprimes.size() > 0) {
        ...
        if (doDisplay_) {
            ...
            TH3D* xyzplot =
                new TH3D("XYZ", "XYZ", 1, minz, maxz + 500, 1, -miny - 500, maxy + 500, 1, minx - 500, maxx + 500);
