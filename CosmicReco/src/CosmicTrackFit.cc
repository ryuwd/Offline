...
CosmicTrackFit::CosmicTrackFit(const Config &conf) : _Npara(conf.Npara()),
													 _diag(conf.diag()),
													 _debug(conf.debug()),
...
void CosmicTrackFit::FitAll(const char *title, CosmicTrackFinderData &trackData, CosmicTrack *cosmictrack)
{
	...
	for (size_t f1 = 0; f1 < nHits; ++f1)
	{
		...
		if (!use_hit(*hitP1) && hitP1->nStrawHits() < _minnsh)
			continue;
	...
	while (niter < _maxniter && converged == false)
	{
		...
		for (size_t f4 = 0; f4 < nHits; ++f4)
		{
			...
			if (((!use_hit(*hitP2)) && (hitP2->nStrawHits() < _minnsh)))
				continue;
		...
		if (abs((a0) - (InitParams.A0)) > _max_position_deviation or abs((b0) - (InitParams.B0)) > _max_position_deviation)
		{
			continue;
		}
	...
	if (cosmictrack->converged and _diag > 0)
	{
		for (size_t f5 = 0; f5 < nHits; ++f5)
		{
			...
			// Remove bad tracks:
			if (abs(newRx) / ErrorsXY[0] > _maxpull or abs(newRy) / ErrorsXY[1] > _maxpull)
			{
				cosmictrack->converged = false;
			}
...
void CosmicTrackFit::ConvertFitToDetectorFrame(CosmicTrackFinderData &trackData, TrackAxes axes, XYZVec Position, XYZVec Direction, CosmicTrack *cosmictrack, bool seed, bool det)
{
	...
	*/
	...
	if (seed == true)
	{ //is this the local coords at start?
		if (det == true)
		{ // is this detector frame?
...
{
	if (track.Diag.FinalChiTot < _max_seed_chi2)
		return true;
	else
		return false;
}
...
void CosmicTrackFit::DriftFit(CosmicTrackFinderData &trackData, StrawResponse const& _srep)
{
	...
	if (endresult.NLL != 0)
	{
		trackData._tseed._track.minuit_converged = true;
	}
